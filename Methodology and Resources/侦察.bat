...
* Using Shodan (https://www.shodan.io/) to detect similar app
  ```bash
  $ --nmap (https://github.com/glennzw/shodan-hq-nse)
...
* Using The Harvester (https://github.com/laramies/theHarvester)
  ```python
>>>theHarvester.py -b all -d domain.com
  ```
## Active
...
$  -w wordlist
$  -t threads
$ <> [!] subdomain :: ./gobuster -m dns -w subdomains.txt -u google.com -i || gobuster -w wordlist -u URL -r -e</>
  ```
...
###  3. Explore and understand available functionality, noting areas that correspond to vulnerability types
    ```bash
$ --.c -.h --Burp_Proxy.config on port:: [8080] (in .bashrc):
...
* Launch a Nikto scan in case you missed something
  ```powershell
PS>  nikto -h http://domain.example.com
  ```
* Payment 
1001 1111
- [@gwendallecoguic](https://twitter.com/gwendallecoguic/status/988138794686779392)
> { if /web/app (You) .test [USE] .ext\  
$ gateway -check /.doc *find .test -cred [INtEGEr];
$ ####purchase something
'&&' {if /web/app [NO] -disable .test.mode then, free };
